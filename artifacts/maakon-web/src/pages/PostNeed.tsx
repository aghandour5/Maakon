...
import { CheckCircle2, ChevronLeft, ChevronRight, AlertCircle, Info } from "lucide-react";
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        ...
        >
          ...
          <Button 
            className="w-full h-12 text-lg hover-elevate bg-primary"
            onClick={() => setLocation('/')}
          >
            {t('return_home')}
          </Button>
    ...
    <div className="min-h-screen bg-secondary/30 flex flex-col">
      ...
      <main className="flex-1 mt-16 w-full max-w-lg mx-auto p-4 sm:p-6 flex flex-col">
            ...
            {[1, 2, 3].map(num => (
              <div 
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors duration-300 ${
                  step >= num ? 'bg-primary text-primary-foreground shadow-md' : 'bg-secondary border-2 border-border text-muted-foreground'
                }`}
              >
                {num}
              </div>
          ...
          <AnimatePresence custom={1} mode="wait">
            {step === 1 && (
              <motion.div
                ...
                className="bg-card rounded-3xl p-6 shadow-sm border border-border flex flex-col gap-6 absolute inset-0"
              >
                <h2 className="text-2xl font-bold text-foreground mb-2">{t('step_1_need')}</h2>
            ...
            {step === 3 && (
              <motion.div
                ...
                exit="exit"
                transition={{ type: "spring", stiffness: 300, damping: 30 }}
                className="bg-card rounded-3xl p-6 shadow-sm border border-border flex flex-col gap-5 absolute inset-0 overflow-y-auto"
              >
                ...
                <div>
                  ...
                  <select 
                    className="w-full p-4 rounded-xl bg-background border-2 border-border focus:border-primary outline-none"
                ...
                <div className="bg-info/10 border border-info/20 p-4 rounded-xl mt-2">
                  <div className="flex gap-3">
                    <Info className="w-5 h-5 text-info shrink-0 mt-0.5" />
                    <div>
                      <label className="block text-sm font-bold text-info-foreground mb-1">{t('exact_address')}</label>
                      <p className="text-xs text-info-foreground/80 mb-3">{t('exact_address_private')}</p>
                      <input 
                        type="text"
        ...
        <div className="mt-8 flex gap-4 pt-4 border-t border-border/50">
          ...
          <Button 
            className="h-14 flex-1 rounded-2xl text-lg font-bold bg-primary hover:bg-primary/90 shadow-lg hover-elevate shadow-primary/20 transition-all"
