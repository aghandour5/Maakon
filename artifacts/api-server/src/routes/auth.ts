...
router.post("/auth/supabase-login", loginLimiter, async (req: Request, res: Response) => {
  ...
  if (!parsed.success) {
    logger.warn({ err: parsed.error, path: req.originalUrl }, "Validation failed for supabase login");
...
router.post("/posts/draft", draftPostLimiter, async (req: Request, res: Response) => {
  ...
  if (!parsed.success) {
    logger.warn({ err: parsed.error, path: req.originalUrl }, "Validation failed for draft post creation");
